...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        if 'label_edge' in sample.keys():
            sample['label_edge'] = F.resize(sample['label_edge'], self.size, Image.NEAREST)
...
class RandomScale(object):
    ...
    def __call__(self, sample):
      ...
      #random.uniform:随机生成下一个实数，它在 [x, y] 范围内。
        ...
        if 'label_edge' in sample.keys():
            sample['label_edge'] = F.resize(sample['label_edge'], size, Image.NEAREST)
