...
class UniFormer(nn.Module):
    ...
    """
    def __init__(self, depth=[3, 4, 8, 3], img_size=(480,480), in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        if conv_stem:
            ...
            self.patch_embed2 = PatchEmbed(
                img_size=(img_size[0] // 4,img_size[1]//4), patch_size=2, in_chans=embed_dim[0], embed_dim=embed_dim[1])
            self.patch_embed3 = PatchEmbed(
                img_size=(img_size[0] // 8,img_size[1]//8), patch_size=2, in_chans=embed_dim[1], embed_dim=embed_dim[2])
            self.patch_embed4 = PatchEmbed(
                img_size=(img_size[0] // 16,img_size[1]//16), patch_size=2, in_chans=embed_dim[2], embed_dim=embed_dim[3])
    ...
    def forward_features(self, x):
        x = self.patch_embed1(x)  #64 120 460
        ...
        x = self.patch_embed2(x)  #大小的缩放和通道数的转变
...
@register_model
def uniformer_small(in_chans,pretrained=False, **kwargs):
    model = UniFormer(
        depth=[3, 4, 8, 3],
        embed_dim=[64, 128, 320, 512], in_chans=in_chans,head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    if pretrained:
        print('load pretrained')
        model.load_state_dict(torch.load('/home/yangenquan/Disk/download/uniformer_small_tl_384.pth'),strict=False)
    return model
...
def uniformer_base(pretrained=True, **kwargs):
    ...
    if pretrained:
        print('yes')
        model.load_state_dict(torch.load('/home/yangenquan/Disk/download/uniformer_base_tl_384.pth'),strict=False)
    return model
...
if __name__=='__main__':
    ...
    # net = PATM_BAB(3,3,3,3,3).cuda()
    # net = uniformer_base(pretrained=True).cuda(0)
    net = uniformer_small(pretrained=True)
    # print(type(image))
    # out = net(ndsm,ndsm1,ndsm2,ndsm3)
    out = net(image)
    # print(out.shape)
    # for i in out:
    #     print(i.shape)